...
void make_guess(void) {
    ...
    if (setups_data.count == 4 && setups_data.last_wlength == 0x10 && setups_data.cnt_ff == 0 && setups_data.cnt_02 == 2) {
        setups_data.detected_os = OS_MACOS;
        return;
    }
    ...
    if (setups_data.cnt_ff >= 1 && setups_data.cnt_02 == 0 && setups_data.cnt_04 == 0) {
        ...
        if (setups_data.cnt_40 > 0) {
            setups_data.detected_os = OS_NINTENDO_SWITCH;
        } else {
            setups_data.detected_os = OS_LINUX;
        }
        return;
